...
const ModalEditDepartment = ({ isOpen, onClose, department }) => {
    const { permissions } = usePage().props;
    const { data, setData, post, processing, errors, reset } = useForm({
        id: department?.id || "",
        name: department?.name || "",
        status: department?.status?.toString() || "1",
    });
    ...
    const handleSubmit = (e) => {
        ...
        post(route("master.department.store"), {
            preserveScroll: true,
            onSuccess: () => {
                toast.success("Berhasil mengubah departement!");
                onClose();
                reset();
            },
            onError: () => {
                toast.error("Gagal mengubah departement.");
            },
        });
    };

    const handleDelete = () => {
        if (confirm("Yakin ingin menghapus departement?")) {
            router.delete(route("master.department.destroy", department?.id), {
                onSuccess: () => {
                    toast.success("Berhasil menghapus departement!");
                    onClose();
                },
                onError: () => {
                    toast.error("Gagal menghapus departement.");
                },
            });
        }
    };
